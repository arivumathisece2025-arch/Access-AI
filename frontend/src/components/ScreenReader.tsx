...
export default function ScreenReader() {
  ...
  return <motion.div className="feature-card-glass" role="tabpanel" id="panel-reader" aria-labelledby="tab-reader" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
    ...
    <AnimatePresence>{result && <motion.div className="result-card" aria-live="polite" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}><div className="result-section"><div className="result-label"><Sparkles size={14} aria-hidden="true" /> Scene Description</div><p className="result-text">{result.caption}</p></div><div className="result-divider" /><div className="result-section"><div className="result-label"><Type size={14} aria-hidden="true" /> Extracted Text</div><p className="result-text text-slate-300">{result.extracted_text}</p></div><div className="result-meta"><span>{result.elapsed_s}s total</span><span>·</span><span>{result.inference_s ?? '—'}s inference</span><span>·</span><span>{result.cached ? 'cached' : 'fresh'}</span></div><div className="result-actions"><button className="btn-action" onClick={() => void speakWithFallback(result.full_description)}><Volume2 size={16} aria-hidden="true" /> Read Aloud</button><button className="btn-action secondary" onClick={cancelSpeech}><Square size={16} aria-hidden="true" /> Stop</button></div></motion.div>}</AnimatePresence>
  </motion.div>;
}
